...
def compose_test_ids(val):
    """
    Generate readable test ids for test cases of the form generated by
    generate_compose_test_cases().
    """
    try:
        if type(val) in (dict, str):
            raise TypeError
        return "({})".format(", ".join(tuple(f.__name__ for f in val)))
    except (AttributeError, TypeError):
        return repr(val)


@pytest.mark.parametrize(
    "compose_args,args,kwargs,expected",
    generate_compose_test_cases(),
    ids=compose_test_ids
)
def test_compose(compose_args, args, kwargs, expected):
    assert compose(*compose_args)(*args, **kwargs) == expected
...
@pytest.mark.parametrize(
    "pipeline_args,args,kwargs,expected",
    generate_pipeline_test_cases(),
    ids=compose_test_ids
)
def test_pipeline(pipeline_args, args, kwargs, expected):
    assert pipeline(*pipeline_args)(*args, **kwargs) == expected
